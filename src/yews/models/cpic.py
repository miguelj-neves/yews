...
__all__ = ['CpicV1', 'CpicV2', 'cpic_v1', 'cpic_v2','cpicv1_dropout', 'CpicDropout','cpic_iberia','CpicIberia']
...
def cpic_iberia(pretrained=False, progress=True, dropout=0.08, path=None, **kwargs):
    r"""Original CPIC model architecture from the
    `"Deep learning for ..." <https://arxiv.org/abs/1901.06396>`_ paper. The
    pretrained model is trained on 60,000 Wenchuan aftershock dataset
    demonstrated in the paper.
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    model = CpicIberia(dropout=dropout, **kwargs)
    if pretrained:
        if path != None:
            model.load_state_dict(load(path)['model'])
        else:
